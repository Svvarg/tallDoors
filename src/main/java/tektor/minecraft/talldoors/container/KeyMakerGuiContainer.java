	...
	public void pressKey(String chosen, boolean b) {
		if (this.outputSlot.getStackInSlot(0) == null) {
			if (!b) {
				...
				for (int i = 0; i < 32; i++) {
					if ( (in[i] != null ) && 
                                             ( in[i].getItem() == TallDoorsBase.key) && 
                                             ( in[i].getItem().getDamage(in[i]) ==0 ) &&
                                             ( in[i].stackTagCompound == null )    
                                                               ) {
				...
				if (ironNeed <= ironFound) {
					ItemStack iron = new ItemStack(TallDoorsBase.key, ironNeed, 0);
					for (int i = 0; i < 32; i++) {
						if (in[i] != null
								&& ( in[i].getItem() == TallDoorsBase.key) 
                                                                && ( in[i].getItem().getDamage(in[i])==0) 
