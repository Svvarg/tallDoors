...
public class DoorModuleWorkbenchGUI extends GuiContainer {
	...
	public void initGui() {
		...
		this.itemNameField2 = new GuiTextField(this.fontRenderer, i + 21,
				j + 84, 90, 12);
	...
	protected void drawGuiContainerForegroundLayer(int param1, int param2) {
		if (mode == 0) {
			...
			fontRenderer.drawString("Door Modules", 8, 6, -1);
			fontRenderer.drawString("Modules:", 8, 17, -1);
			for (int i = off; (i < off + 7 && i < keys.length); i++) {
				int color = -1;
				if (chosen.equals(keys[i]))
					color = 5919952;

				fontRenderer.drawString(keys[i], 8, 27 + (i - off) * 10,
						color);
			}
			fontRenderer.drawString(page + "/" + pages, 48, 107, -1);
			fontRenderer.drawString("Prio " + priority + " " + moduleType,
					8, 122, -1);
			int i = 0;
			for (String s : DoorPartRegistry.getPartForIndex(chosen)
					.getCostAsString()) {
				fontRenderer.drawString(s, 8, 132 + i, -1);
				i = i + 10;
			}
		} else if (mode == 1) {
			for (int i = off2; (i < off2 + 7 && i < keysT.length); i++) {
				...
				fontRenderer.drawString(keysT[i], 8, 7 + (i - off2) * 10,
						color);
			}
			fontRenderer.drawString(pageT + "/" + pagesT, 58, 105, -1);
			...
			for (int i = off3; (i < off3 + 7 && i < keysT2.length); i++) {
				...
				fontRenderer.drawString(keysT2[i], 140, 7 + (i - off3) * 10,
						color);
			}
			fontRenderer.drawString(pageT2 + "/" + pagesT, 190, 105, -1);
		}
	}
	...
	protected void actionPerformed(GuiButton button) {
		...
		case 0: {
			...
			try {
				outputStream.writeUTF("module");
				outputStream.writeInt(te.xCoord);
				outputStream.writeInt(te.yCoord);
				outputStream.writeInt(te.zCoord);
				outputStream.writeInt(this.priority);
				outputStream.writeUTF(this.chosen);
				outputStream.writeUTF(this.moduleType);
				outputStream.writeUTF(this.texture1);
				outputStream.writeUTF(this.texture2);
